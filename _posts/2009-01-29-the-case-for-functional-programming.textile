...
* "Prior Comparisons Of Language Levels":#prior-comparisons
...
One could think of a declarative language as one which expresses computations as _a hierarchy of result descriptions_.

Following this line of thinking, functional programming expresses computations as _a hierarchy of function applications_, where each function is responsible of producing a result.
...
Since the mechanism of function application, or in other words, function _call_, is universally known, functional programming is far from being inaccessible. And that's precisely what might come as a surprise to one who has heard functional programming is a weird way of thinking of the process of writing software.

h4. Some Haskell Dialect

Remark how this code feels like mathematical equations.

Note that I have taken the liberty to personalize Haskell's syntax so that it reads more easily to the eyes of a java-like language programmer:
...
base            = mix([Flour(Cup(2)), BakingPowder(Teaspoon(2)), Salt(Teaspoon(1/2))])
{% endhighlight %}

This code uses only one programming technique, which is the already mentioned mechanism of function application.

Each function name is written on the left side of @=@, which means "... is described as ...". Obviously, the right side of @=@ describes what computations this function depends on to produce its result.
...
Aside from the fact that conventional languages are said to be imperative, how could one define this concept?

One could think of an imperative language as one which expresses computations as _a sequence of manipulations which must imperatively be followed, affecting the state of memory so as to progressively build a result_.

When a programmer uses the imperative style, he is mostly concerned with the interactions of the manipulations with the progressively-built result.
...
h2(#prior-comparisons). Prior Comparisons Of Language Levels
...
h4. On Imperative Programming Languages
...
* "Can Programming Be Liberated from the von Neumann Style?":http://google-it-for-now.com
